...
impl<T: StakingConfig> StakingPallet<T> {
    ...
    pub fn stake(&mut self, who: T::AccountId, amount: T::Balance) -> Result<(), &'static str> {
        ...
        if free_balance < amount {
            return Err("Not enough free balance");
        }

        let new_free_balance = free_balance.checked_sub(&amount).ok_or("Underflow")?;

        let current_staked_balance = self
            .staked_balances
            .get(&who)
            .copied()
            .unwrap_or(T::Balance::zero());

        let new_staked_balance = current_staked_balance
            .checked_add(&amount)
            .ok_or("Overflow")?;
    ...
    pub fn unstake(&mut self, who: T::AccountId, amount: T::Balance) -> Result<(), &'static str> {
        ...
        if staked_balance < amount {
            return Err("Not enough staked balance");
        }

        let new_staked_balance = staked_balance.checked_sub(&amount).ok_or("Underflow")?;

        let current_free_balance = self
            .free_balances
            .get(&who)
            .copied()
            .unwrap_or(T::Balance::zero());

        let new_free_balance = current_free_balance
            .checked_add(&amount)
            .ok_or("Overflow")?;
